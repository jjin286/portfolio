export const navItems = [
  { name: "About", link: "#about" },
  { name: "Projects", link: "#projects" },
  { name: "Technologies", link: "#tech" },
  { name: "Contact", link: "#contact" },
];

export const gridItems = [
  {
    id: 1,
    title: "Hi, my name's JaeWon!",
    description: "Fullstack Developer based in California",
    className: "lg:col-span-3 md:col-span-6 md:row-span-4 md:min-h-[60vh] z-30",
    imgClassName: "rounded-full aspect-square object-cover md:w-3/5 mx-auto md:bottom-12 left-0 right-0  absolute z-30",
    titleClassName: "items-center z-30",
    img: "/jaewon.jpg",
    spareImg: "",
  },
  {
    id: 2,
    title: "I'm a very flexible thinker that is open to new ideas",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    imgClassName: "",
    titleClassName: "justify-center text-center py-10",
    img: "",
    spareImg: "",
  },
  {
    id: 3,
    title: "I constantly try to improve my tech stack",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    imgClassName: "",
    titleClassName: "justify-center",
    img: "",
    spareImg: "",
  },
  {
    id: 4,
    title: "Tech enthusiast with a passion for",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-start",
    img: "/grid.svg",
    spareImg: "/b4.svg",
  },

  {
    id: 5,
    title: "Currently building a pantry management web application",
    description: "Wasteless Kitchen",
    className: "md:col-span-3 md:row-span-2",
    imgClassName: "absolute right-0 bottom-0 md:w-96 w-60",
    titleClassName: "justify-center md:justify-start lg:justify-center",
    img: "/b5.svg",
    spareImg: "/grid.svg",
  },
  {
    id: 6,
    title: "Do you want to start a project together?",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-center md:max-w-full max-w-60 text-center",
    img: "",
    spareImg: "",
  },
];

export const projects = [
  {
    id: 1,
    title: "Jobly - Online Job board",
    des: "Search for your next job opportunity with Jobly. Make finding a perfect match smooth for employees and employers alike.",
    img: "/jobly.PNG",
    iconLists: ["/re.svg", "Express.svg", "Bootstrap.svg", "/Javascript.svg","/PostgresSQL.svg"],
    link: "https://react-jobly-wvgv.onrender.com",
  },
  {
    id: 2,
    title: "Warbler - Social Media Platform",
    des: "Connect with your friends with Warbler. Keep in touch and meet new people with a click of a button.",
    img: "/warbler.PNG",
    iconLists: ["/re.svg", "/Flask.svg", "Bootstrap.svg", "/Javascript.svg", "/Python.svg", "/PostgresSQL.svg"],
    link: "https://warbler-bxs4.onrender.com",
  },
  {
    id: 3,
    title: "ShareBnB - Online Marketplace for rent",
    des: "Plan your next trip with ShareBnb. Find your perfect booking from a variety of listings.",
    img: "/sharebnb.PNG",
    iconLists: ["/re.svg", "Express.svg", "Bootstrap.svg", "/Javascript.svg","/PostgresSQL.svg", "/Sequelize.svg", "/amazon-s3.svg"],
    link: "https://react-sharebnb.onrender.com",
  },
];

export const techRight = [
  {
    id: 1,
    name: "bootstrap",
    img: "/Bootstrap.svg",
    nameImg: "/bootstrapName.svg",
  },
  {
    id: 2,
    name: "django",
    img: "/Django.svg",
    nameImg: "/djangoName.svg",
  },
  {
    id: 3,
    name: "css3",
    img: "/CSS3.svg",
    nameImg: "CSS3",
  },
  {
    id: 4,
    name: "express",
    img: "/Express.svg",
    nameImg: "express",
  },
  {
    id: 5,
    name: "flask",
    img: "/Flask.svg",
    nameImg: "Flask",
  },
  {
    id: 6,
    name: "html5",
    img: "/HTML5.svg",
    nameImg: "HTML5",
  },
  {
    id: 7,
    name: "jest",
    img: "/Jest.svg",
    nameImg: "jest",
  },
  {
    id: 8,
    name: "javascript",
    img: "/Javascript.svg",
    nameImg: "Javascript",
  },
  {
    id: 9,
    name: "jasmine",
    img: "/Jasmine.svg",
    nameImg: "Jasmine",
  },
];

export const techLeft = [
  {
    id: 10,
    name: "next.js",
    img: "/Next.js.svg",
    // nameImg: "",
    nameImg: "/next.jsName.svg",
  },
  {
    id: 11,
    name: "node.js",
    img: "/Node.js.svg",
    nameImg: "node.js",
  },
  {
    id: 12,
    name: "postgressql",
    img: "/PostgresSQL.svg",
    nameImg: "PostgreSQL",
  },
  {
    id: 13,
    name: "python",
    img: "/Python.svg",
    nameImg: "python",
  },
  {
    id: 14,
    name: "react",
    img: "/React.svg",
    nameImg: "React",
  },
  {
    id: 15,
    name: "sequelize",
    img: "/Sequelize.svg",
    nameImg: "Sequelize",
  },
  {
    id: 16,
    name: "sqlalchemy",
    img: "/SQLAlchemy.svg",
    nameImg: "SQLAlchemy",
  },
  {
    id: 17,
    name: "tailwind css",
    img: "/Tailwind CSS.svg",
    nameImg: "tailwind css",
  },
  {
    id: 18,
    name: "typescript",
    img: "/TypeScript.svg",
    nameImg: "typescript",
  },
];

export const socialMedia = [
  {
    id: 1,
    img: "/git.svg",
    link: "https://github.com/jjin286"
  },
  {
    id: 2,
    img: "/link.svg",
    link: "https://www.linkedin.com/in/jaewon-jin/"
  },
];